...
//For text appearing casually in a scene opencv 3.0 has a text module that deals with
// scene text detection. It uses Neuman and Matas algorithms that came out in 2012
// Opencv 3.0 uses connnected component analysis and searching for extremal regions.
// Extremal regions are connected areas that are characterize by uniform intensities. The
// stability of the region is measured by resistance to thesholding variance.
// Apply thresholding to image and get an image A, increase the threshold again and apply
// it to A, compare the image B with A, if region is same as it was( in area and shape) in 
// A, then add it to the same branch in the tree, if a regions appears split, create two 
// branches under the tree and add these regions to these two regions, then apply threshol
//ding on each branch and populate the tree
// MSERs Maximally stable external regions are regions where area remains stable in a wide
//variety of thresholds.

// Neumann and Matas algorithm sends all the extremal regions to a classifier that works 
// in two stages(to detect whether an extremal regions has text).
// The first stage incrementally computes descriptors(bounding box, perimeter, area, euler 
// number) for a regions and these descriptors are sent to a classifier, if the classifiers
// predicts that this regions could be text, these are then sent to stage 2 which selects
// some other features like whole area ratio, hull ratio etc, this helps discard non text 
// regions better. 
// this is implemented by ERFilter class.
//Finally all characters must be grouped in text blocks


// ...................... Functions Follow ...........................

// Since ERFilter works separately in each image channel, we separate each channel into 
// a vector of Mat
